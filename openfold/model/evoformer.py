...
from openfold.utils.tensor_utils import chunk_layer, permute_final_dims
...
from openfold.utils.profiling import prof_memory
from openfold.model.embedders import PairFactor
import pdb
...
class MSATransition(nn.Module):
    ...
    def forward(
        ...
        # start to record max allocated memory in msa_transiton
        ...
        if GlobalConfig.DEV_PROF_MEM_MODE:
            logger.info(
                f"allocated memory before del mask: {torch.cuda.memory_allocated(device=0)}"
            )
        ...
        if GlobalConfig.DEV_PROF_MEM_MODE:
            logger.info(
                f"allocated memory after del mask: {torch.cuda.memory_allocated(device=0)}"
            )

        # print max allocated memory in msa_transition
        if GlobalConfig.DEV_PROF_MEM_MODE:
            logger.info(
                f"max allocated memory in msa_transition: {torch.cuda.max_memory_allocated(device=0)}"
            )
        ...
        if GlobalConfig.DEV_PROF_TIME_MODE:
            ...
            logger.info(f"run msa_transition time: {starter.elapsed_time(ender)/1000}s")
...
class EvoformerBlockCore(nn.Module):
    """
    shared core implementations for ExtraMsaStack in Alg.18 and EvoformerStack in Alg.6
    """
    ...
    def __init__(
        ...
        if not self.is_multimer:
            ...
            if GlobalConfig.PAIR_FACTOR:
                self.u_outer = VariableLinear(crop_size,factor_rank)
                self.v_outer = VariableLinear(crop_size,factor_rank)


        self.tri_mul_out = TriangleMultiplicationOutgoing(
            c_z,
            c_hidden_mul,
            rank_factor=factor_rank,
            crop_size=crop_size,
        )
    ...
    def forward(
        ...
        if not self.is_multimer:
            if GlobalConfig.PAIR_FACTOR:
                msa_info = self.outer_product_mean(m, mask=msa_mask, chunk_size=chunk_size)
                z = z_.clone()
                z[0] = z[0]+ self.u_outer(msa_info.transpose(-1,-2)).transpose(-1,-2)
                z[1] = z[1]+ self.v_outer(msa_info.transpose(-1,-3)).transpose(-2,-3).transpose(-2,-1)
            else:
                z = z_ + self.outer_product_mean(m, mask=msa_mask, chunk_size=chunk_size)
        else:
            z = z_.clone()
...
class EvoformerBlock(nn.Module):
    def __init__(
        ...
        crop_size: int=0,
        is_multimer: bool = False,
        use_msa_att_col: bool = True,
        model_version: str = "v1",
    ):
        super(EvoformerBlock, self).__init__()
        self.is_multimer = is_multimer
        self.use_msa_att_col = use_msa_att_col
    ...
    def forward(
        ...
        if GlobalConfig.DEV_PROF_MEM_MODE or GlobalConfig.DEV_PROF_MEM_MAIN_MODE:
            ref_memory_reserved, ref_max_memory_allocated = prof_memory()
            starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(
                enable_timing=True
            )
            starter.record()
        ...
        if self.is_multimer:
            ...
            z = z_.clone()
        m = m + self.msa_dropout_layer(self.msa_att_row(m, z, mask=msa_mask, chunk_size=chunk_size))
        if self.use_msa_att_col:
        ...
        if GlobalConfig.DEV_PROF_MEM_MODE or GlobalConfig.DEV_PROF_MEM_MAIN_MODE:
            memory_reserved, max_memory_allocated = prof_memory()
            print(
                f"evoformer block: "
                f"memory_reserved={round(memory_reserved, 2)}, "
                f"memory_reserved_cost={round(memory_reserved-ref_memory_reserved, 2)}, "
                f"max_memory_allocated_cost={round(max_memory_allocated-ref_max_memory_allocated, 2)}"
            )
            print(f'evoformer block end max allocate mem {max_memory_allocated}')
            ender.record()
            torch.cuda.synchronize()  # WAIT FOR GPU SYNC
            logger.info(
                f"run evoformer block time: {starter.elapsed_time(ender)/1000}s"
            )
...
class EvoformerStack(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pair_factor = PairFactor(crop_size,factor_rank)
    ...
    def forward(
        ...
        """
        # factorize pair mask and z
        inference = False
        if GlobalConfig.PAIR_FACTOR:
            try:
                u,v = self.pair_factor(z)
                mask_diag = torch.diagonal(pair_mask,offset=0,dim1=1,dim2=2)
                u_mask = mask_diag[...,None].repeat(1,1,u.shape[-2])
                v_mask = mask_diag[...,None].repeat(1,1,v.shape[-2])
                pair_mask = torch.concat([u_mask[None,:],v_mask[None,:]],dim=0)
                z = torch.concat([u[None,:],v[None,:]],dim=0)
            except: # inference,没有batchsize
                inference = True
                u,v = self.pair_factor(z[None])
                mask_diag = torch.diagonal(pair_mask[None],offset=0,dim1=1,dim2=2)
                u_mask = mask_diag[...,None].repeat(1,1,u.shape[-2])
                v_mask = mask_diag[...,None].repeat(1,1,v.shape[-2])
                pair_mask = torch.concat([u_mask[None,:],v_mask[None,:]],dim=0)
                z = torch.concat([u[None,:],v[None,:]],dim=0)
        ...
        if GlobalConfig.DEV_PROF_MEM_MODE or GlobalConfig.DEV_PROF_MEM_MAIN_MODE:
            ...
            torch.cuda.synchronize()  # WAIT FOR GPU SYNC
            logger.info(
                f"run evoformer time: {starter.elapsed_time(ender)/1000}s"
            )

        s = self.linear(m[..., 0, :, :])  # [*, N_res, C_s]
        # reconstruct z
        # z [*,N_res,N_res,c_z]
        if GlobalConfig.PAIR_FACTOR:
            u = permute_final_dims(z[0],(2,0,1))
            v = permute_final_dims(z[1],(2,1,0))
            if inference:
                z = permute_final_dims(torch.matmul(u,v),(1,2,0))[0]
            else:
                z = permute_final_dims(torch.matmul(u,v),(1,2,0))
